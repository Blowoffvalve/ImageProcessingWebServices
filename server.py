...
from quart import Quart, request, url_for, send_file, Response, jsonify
...
certs = os.path.sep.join([certdir, "pycacert.pem"])
...
async def json_from_byte_array(bytes):
    return json.loads(bytes.decode(encoding='utf-8').replace("'", '"'))

@app.route("/frameProcessing", methods = ["POST"])
async def frameProcessing():
    """
    ...
    file = await json_from_byte_array(data)
    frame = np.array(file["Frame"], dtype = "uint8")
    configuration = QuicConfiguration(
        is_client=True, alpn_protocols=H3_ALPN
    )
    configuration.load_verify_locations(certs)
    ...
    uri = "http://" + getNextServer() + "/objectClassifier"
    parsed = urlparse(uri)
    ...
    async with connect(
            ...
            http_events = await client.post(
                u"/objectClassifier",
                data=str(json.dumps({"Frame":frame.tolist()})).encode('utf-8')
            )

    # r = requests.post(, headers = headers, json =  )
    """
    for c in cnts:
        print("x")
...
async def classifier():
    ...
    if len(idxs) > 0:
        ...
        return {"label": LABELS[classIDs[i]], "status": 200}
    else:
        return {"status": 200}
