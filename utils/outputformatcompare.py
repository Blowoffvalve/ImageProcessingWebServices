...
def saveImage(imageFormats):
    ...
    for imageFormat in imageFormats:
        ...
        formatTimers[imageFormat] = {"AverageRunTime":np.mean(formatTimer), 
                                     "AverageCPU": np.mean(cpu), 
                                     "AverageMemory":np.mean(memory)}
        print(imageFormat, ": AverageRunTime ", str(np.mean(formatTimer)), " AverageCPU ", str(np.mean(cpu)), 
              " AverageMemory ", str(np.mean(memory)), sep = "")
